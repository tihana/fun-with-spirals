...
class MatrixUtilsSpec extends BaseSpec {
  ...
  "print distance" must {
    (manhattanDistances.take(matrices.size), matrices, steps).zipped foreach { case (manhattanDistance, matrix, step) =>
      s"print out distance for number ${manhattanDistance.number}" in {
        ...
        output must startWith regex s"From location ${manhattanDistance.number} the distance is ${manhattanDistance.distance}${Regex.quote(step)}"
